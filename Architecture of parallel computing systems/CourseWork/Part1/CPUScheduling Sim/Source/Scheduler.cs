...
namespace CPUScheduling_Sim.Source
{
    ...
    internal static class Scheduler
    {
        ...
        private static Processes FCFSSchedule()
        {
            ...
            //Under section comment for output graphics without alpply schedule alg.
        ...
        /// <summary>
        /// Check completion time rich critical time or not for non-preemtive algorithms
        /// </summary>
        /// <param name="processes"></param>
        private static void CalculateNonPreCriticalTime(Processes processes)
        {
            for (int i = 0; i < Processes.Count; i++)
            {
                var ct = FindCompletionTime(processes, i);
        ...
        /// <summary>
        /// Check completion time rich critical time or not for preemtive algorithms
        /// </summary>
        /// <param name="processes"></param>
        private static void CalculatePreCriticalTime(Processes processes)
        {
            for (int i = 0; i < Processes.Count; i++)
            {
                var lastExecution = processes.FindLast(p => p.PID == Processes[i].PID);
                var ct = lastExecution.ArriveTime + lastExecution.CPUTime;
                if (ct > processes[i].CriticalTime)
                    Console.WriteLine("Process" + i);
            }
