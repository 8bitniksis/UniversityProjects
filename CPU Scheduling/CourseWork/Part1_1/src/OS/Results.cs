...
namespace OS
{
    public partial class Results : Form
    {
        ...
        float avg_waiting_c;
        float freeTimes_c;
        float freeTimesPercent_c;
        float avg_turnaround_c;
        int lastValid_c;

        //int interval = 45;
        ScheduleAlgorithm alg;
        Color [] colors;
        ...
        { 
            if (Form_process.RRflag)
            {
                ...
                alg = new ScheduleAlgorithm(InputData.mArrivalsTime, InputData.mBurstsTime, InputData.mPrioritys, InputData.mCriticalsTime, InputData.mQuantumTime, Form_process.no_of_process);
                alg.mTexBox = this.FreeTimeLog_TextBox;
                alg.ScheduleRoundRobin();
                lastValid_c = alg.lastValid;
                avg_turnaround_c = alg.avg_turnaround;
                avg_waiting_c = alg.avg_waiting;
                freeTimes_c = alg.freeTimes;
                freeTimesPercent_c = alg.freeTimesPercent;
                for (int i = 0; i < alg.lastValid; i++)
                {
                    start_c[i] = alg.start[i];
                    end_c[i] = alg.end[i];
                    proc_c[i] = alg.proc[i];
                }
                for (int i = 0; i < Form_process.no_of_process; i++)
                {
                    waitingTime_c[i] = alg.waitingTime[i];
                    turnaroundTime_c[i] = alg.turnaroundTime[i];
                }
            }
            ...
            else if (Form_process.P_non_premptive)
            {
                ...
                alg = new ScheduleAlgorithm(InputData.mArrivalsTime, InputData.mBurstsTime, InputData.mPrioritys, InputData.mCriticalsTime, InputData.mQuantumTime, Form_process.no_of_process);
                alg.mTexBox = this.FreeTimeLog_TextBox;
                alg.SchedulePriorityNonPremptive();
                lastValid_c = alg.lastValid;
                avg_turnaround_c = alg.avg_turnaround;
                avg_waiting_c = alg.avg_waiting;
                freeTimes_c = alg.freeTimes;
            ...
            else if (Form_process.SJF_non_premptive)
            {
                ...
                alg = new ScheduleAlgorithm(InputData.mArrivalsTime, InputData.mBurstsTime, InputData.mPrioritys, InputData.mCriticalsTime, InputData.mQuantumTime, Form_process.no_of_process);
                alg.mTexBox = this.FreeTimeLog_TextBox;
                alg.ScheduleShortestJobFirstNonPremptive();
                lastValid_c = alg.lastValid;
                avg_turnaround_c = alg.avg_turnaround;
                avg_waiting_c = alg.avg_waiting;
                freeTimes_c = alg.freeTimes;
            ...
            else if (Form_process.SJF_premptive)
            {
                ...
                alg = new ScheduleAlgorithm(InputData.mArrivalsTime, InputData.mBurstsTime, InputData.mPrioritys, InputData.mCriticalsTime, InputData.mQuantumTime, Form_process.no_of_process);
                alg.mTexBox = this.FreeTimeLog_TextBox;
                alg.ScheduleShortestJobFirstPremprive();
                lastValid_c = alg.lastValid;
                avg_turnaround_c = alg.avg_turnaround;
                avg_waiting_c = alg.avg_waiting;
                freeTimes_c = alg.freeTimes;
            ...
            else 
            {
                ...
                alg = new ScheduleAlgorithm(InputData.mArrivalsTime, InputData.mBurstsTime, InputData.mPrioritys, InputData.mCriticalsTime, InputData.mQuantumTime, Form_process.no_of_process);
                alg.mTexBox = this.FreeTimeLog_TextBox;
                alg.ScheduleFirstComeFirstServed();
                lastValid_c = alg.lastValid;
                avg_turnaround_c = alg.avg_turnaround;
                avg_waiting_c = alg.avg_waiting;
                freeTimes_c = alg.freeTimes;
        ...
        private void play()
        {
            ...
            float freeTimeCounter = 0;
            if (p == -1)
            {
                this.label3.BackColor = Color.Gray;
                this.label3.Text = "f";
                //freeTimeCounter += end_c[0] - start_c[0];
                Console.WriteLine("Суммарно время простоя: " + freeTimes_c);
                this.label8.Text = freeTimes_c.ToString();
            }
            else
            {
            ...
            for(int i=1; i<=lastValid_c ;i++)
            {
                ...
                l2.TextAlign = l.TextAlign;
                 ...
                 name = "P" + (p+1).ToString();
                //Compute free time in percent
                if (p == -1)
                 { 
                    l2.Text = "f";
                    l2.BackColor = Color.Gray;
                    //freeTimeCounter += (end_c[i] - start_c[i]);
                    //(суммарное время простоя / все время) * 100 = процент простоя
                    Console.WriteLine("Суммарно время простоя: " + freeTimes_c);
                    this.label8.Text = freeTimes_c.ToString();
                ...
                this.label11.Text = freeTimesPercent_c.ToString();
        ...
        private void panel81_Paint(object sender, PaintEventArgs e)
        {

        }

        private void lbl_p1_waitingTime_Click(object sender, EventArgs e)
        {
        ...
        private void FreeTimeLog_TextBox_TextChanged(object sender, EventArgs e)
        {
